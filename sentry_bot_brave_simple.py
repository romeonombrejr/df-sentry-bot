#!/usr/bin/env python3
"""
SEO / Defacement Monitor — DF SentryBot (Brave Search API — Simple edition)
Single-request version. Searches site:<domain>, audits up to --count pages.
Homepage is promoted to position 0 if Brave doesn't return it first.
...
USAGE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  python sentry_bot_brave_simple.py cloudway.com
  python sentry_bot_brave_simple.py cloudway.com site2.com --count 5
  python sentry_bot_brave_simple.py --domains-file clients.txt --count 5
  python sentry_bot_brave_simple.py cloudway.com --teams-webhook WEBHOOK_URL
  python sentry_bot_brave_simple.py cloudway.com --api-key YOUR_KEY --headless
...
# ── Brave Search (single request) ────────────────────────────────────────────

def brave_search(domain: str, api_key: str, count: int = 10) -> list[dict]:
    ...
    resp = requests.get(
        ...
        params={"q": f"site:{domain}", "count": count, **loc},
        timeout=15,
    )
    ...
    # Promote homepage to position 0
    homepage_variants = {
        f"https://{domain}".rstrip("/"),
        f"https://www.{domain}".rstrip("/"),
        f"http://{domain}".rstrip("/"),
        f"http://www.{domain}".rstrip("/"),
    }
    results.sort(key=lambda r: 0 if r["url"].rstrip("/") in homepage_variants else 1)
...
async def run(domain: str, api_key: str, count: int, headless: bool,
    ...
    print(f"\n{SEP}")
    print(f"{_B}  DF SentryBot (Brave/Simple)  —  site:{domain}  [{count} page(s)]{_X}")
    ...
    # ── 1. Brave search ───────────────────────────────────────────────────
    print(f"\n  [1/2]  Fetching Brave index data for site:{domain} …")
    try:
...
def main():
    ...
    if not parsed["api_key"]:
        print(
            ...
            "    CMD        : set BRAVE_API_KEY=your_key_here\n\n"
            "  Or pass it directly:\n"
            "    python sentry_bot_brave_simple.py cloudway.com --api-key YOUR_KEY\n"
        )
        sys.exit(1)
